...
class appStore extends Component {
  state = {searchInp: '', activeTab: tabsList[0].tabId}
  ...
  render() {
    const {searchInp, activeTab} = this.state
    const appItems = this.getFilteredAppItems()

    const flitedItemsBySearch = appItems.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInp.toLowerCase()),
    )
    return (
        ...
        <div className="main-container">
          ...
          <ul className="tabs-container">
            {tabsList.map(eachList => (
              <TabItems
                tabsItems={eachList}
                changingToNewTab={this.changingToNewTab}
                key={eachList.tabId}
                isActive={eachList.tabId === activeTab}
              />
            ))}
          </ul>
          <ul className="apps-list">
            {flitedItemsBySearch.map(eachList => (
              <AppItem appsLists={eachList} key={eachList.appId} />
            ))}
          </ul>
        </div>
      </div>
